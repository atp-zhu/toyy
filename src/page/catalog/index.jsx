...
import { getGoodsList } from '../admin';

// catalog页面
function Catalog(){
  const [lists,setLists]=useState(getGoodsList)
  const [types,setType]=useState('all')
  const filterList=(type)=>{
    if(type==='all'){
      setType('all')
      return setLists(getGoodsList())
    }else if(type==='wooden'){  
      setType('wooden')
      return setLists(getGoodsList().filter(item=>item.type==='wooden'))
    }else if(type==='stuffed'){
      setType('stuffed')
      return setLists(getGoodsList().filter(item=>item.type==='stuffed'))
    }
  }
